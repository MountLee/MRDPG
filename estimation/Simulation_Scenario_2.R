...
#### result_mase =   array(NA, c( length(n_grid_X), length(n_grid_Y), length(L_grid),length(d_grid), NMC))
result_twist =   array(NA, c( length(n_grid_X), length(n_grid_Y), length(L_grid),length(d_grid), NMC))
    ...
    for(ind_L in 1: length(L_grid) ){
      ...
      for(ind_d in 1: length(d_grid) ){
        ...
        for(iter  in 1:NMC){ 
          ...
          result_twist[ind_n_1, ind_n_2, ind_L, ind_d, iter] = frobenius(P_hat_9, P_true)
...
value= c( apply(result_hetero, c(1,2,3,4), mean)[,,,],
          apply(result_hosvd, c(1,2,3,4), mean)[,,,],
          apply(result_hooi, c(1,2,3,4), mean)[,,,],
          apply(result_twist, c(1,2,3,4), mean)[,,,],
...
sd  = c(  apply(result_hetero, c(1,2,3,4), sd)[,,,],
          apply(result_hosvd, c(1,2,3,4), sd)[,,,],
          apply(result_hooi, c(1,2,3,4), sd)[,,,],
          apply(result_twist, c(1,2,3,4), sd)[,,,],
...
layer = rep(c(10,20,30,40), times = 8)

Method = rep(c('TH-PCA', 'HOSVD', 'HOOI','TWIST', 'SASE', 'UASE', 'MLE','COA' ),each = 4) 

dff_1 = data.frame( layer = layer, Method = Method, value = value, sd = sd)

dff_1$Method <- factor(dff_1$Method, levels = c('TH-PCA', 'HOSVD', 'HOOI','TWIST', 'SASE', 'UASE', 'MLE','COA'))

color_list <- setNames(c("#66c2a5", "#8da0cb", "#e78ac3", "#a6d854", "#ffd92f", "#e5c494",  "#484B8A", "#C35817"),
                       levels(dff_1$Method))
...
value= c( apply(result_hetero, c(1,2,3,4), mean)[,,,],
          ...
          apply(result_svd, c(1,2,3,4), mean)[,,,],
          apply(result_uase, c(1,2,3,4), mean)[,,,],
          #### apply(result_mase, c(1,2,3,4), mean)[,,,],
          apply(result_flex, c(1,2,3, 4), mean)[,,,],
          apply(result_MultiNeSS_ada, c(1,2,3, 4), mean)[,,,]
...
sd  = c(  apply(result_hetero, c(1,2,3,4), sd)[,,,],
          ...
          apply(result_svd, c(1,2,3,4), sd)[,,,],
          apply(result_uase, c(1,2,3,4), sd)[,,,],
          #### apply(result_mase, c(1,2,3,4), sd)[,,,],
          apply(result_flex, c(1,2,3, 4), sd)[,,,],
          apply(result_MultiNeSS_ada, c(1,2,3, 4), sd)[,,,]
)
...
dff_1_1$Method <- factor(dff_1_1$Method, levels = c('TH-PCA', 'HOSVD', 'HOOI','TWIST', 'SASE', 'UASE', 'MLE','COA'))

color_list <- setNames(c("#66c2a5", "#8da0cb", "#e78ac3", "#a6d854", "#ffd92f", "#e5c494",  "#484B8A", "#C35817"),
                       levels(dff_1_1$Method))
...
dimension = rep( c(2, 4, 6, 8), times = 8)

Method = rep(c('TH-PCA', 'HOSVD', 'HOOI','TWIST', 'SASE', 'UASE', 'MLE','COA' ),each = 4) 

dff_2 <- data.frame( dimension = dimension, Method = Method, value = value, sd = sd)

dff_2$Method <- factor(dff_2$Method, levels = c('TH-PCA', 'HOSVD', 'HOOI','TWIST', 'SASE', 'UASE', 'MLE','COA'))
...
dff_2_1$Method <- factor(dff_2_1$Method, levels = c('TH-PCA', 'HOSVD', 'HOOI','TWIST', 'SASE', 'UASE', 'MLE','COA'))

color_list <- setNames(c("#66c2a5", "#8da0cb", "#e78ac3", "#a6d854", "#ffd92f", "#e5c494",  "#484B8A", "#C35817"),
                       levels(dff_2_1$Method))
...
node = rep( c(50, 100, 150, 200), times = 8)
...
dff_3$Method <- factor(dff_3$Method, levels = c('TH-PCA', 'HOSVD', 'HOOI','TWIST', 'SASE', 'UASE', 'MLE','COA'))
...
Method = rep(c('TH-PCA', 'HOSVD', 'HOOI', 'SASE', 'UASE' ),each = 4) 

dff_4 <- data.frame( node = node, Method = Method, value = value, sd = sd)


dff_4$Method <- factor(dff_4$Method, levels = c('TH-PCA', 'HOSVD', 'HOOI', 'SASE', 'UASE'))

color_list <- setNames(c("#66c2a5", "#8da0cb", "#e78ac3", "#ffd92f", "#e5c494"),
                       levels(dff_4$Method))
...
ggarrange(
plott_1, plott_1_1, plott_2, plott_2_1, plott_3, plott_4,
labels = c("A", "B", "C", "D", "E", "F"),
ncol = 2, # Number of columns in the arrangement
nrow = 3  # Number of rows can be inferred from number of plots and columns, but specifying can avoid mistakes
)


dev.copy2pdf(file=paste("/Users/chris/Desktop/MRDPG-main/estimation/scenario_2_dirichlet.pdf",sep = ''),width = 8,height = 8)
